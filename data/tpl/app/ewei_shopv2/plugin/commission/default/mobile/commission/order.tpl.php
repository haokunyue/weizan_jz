...
<script id='tpl_commission_order_list' type='text/html'>
    ...
    <div class='fui-list-group'>
        ...
        <?php  } ?>
        <div class='fui-list'>
                    <span class="left" style="font-size: .55rem;color: #999;line-height: .95rem"><?php  if(!p('offic')) { ?>分销等级：<%order.level%><?php  } ?><br>订单编号：<%order.ordersn%><br>下单时间：<%order.createtime%>
                    </span>
            <!--<span class="right">+<%order.commission%><br><span><%order.status%></span></span>-->
            <?php  if(!empty($this->set['openorderdetail']) || !empty($this->set['openorderbuyer'])) { ?>
            <!--<span class="remark"></span>-->
        ...
        <div class='fui-list' style="font-size: .55rem;justify-content: flex-end;border-top: .025rem solid #ebebeb;">
            预计佣金：<span class="right" style="font-size: .8rem">+<%order.commission%></span>
        </div>
    </div>
    <%/each%>
</script>
