...
<div class="fui-page fui-page-current page-commission-index ">
	<div class="fui-header">
		...
		<div class="fui-header-right"><a href="<?php  echo mobileUrl('commission/log')?>"><?php  echo $this->set['texts']['commission_detail']?></a></div>
	</div>
	...
	<div class='fui-cell-group'>
	    ...
	    <div class='fui-cell'>
		<div class='fui-cell-icon'><i class='icon icon-qianbao'></i></div>
		<div class='fui-cell-label' style='width:auto'><?php  echo $this->set['texts']['commission_ok']?></div>
		<div class='fui-cell-info' ></div>
		<div class='fui-cell-remark noremark' ><span class="text-orange"><?php  echo number_format($member['commission_ok'],2)?> <?php  echo $this->set['texts']['yuan']?></span></div>
	    </div>
	</div>
	...
	<div class='fui-according-group'>
		...
		<div class='fui-according-content'>
		    ...
		    <?php  echo $this->set['texts']['commission']?>可<?php  echo $this->set['texts']['withdraw']?>。结算期内，买家退货，
		    <?php  echo $this->set['texts']['commission']?>将自动扣除。
		    <?php  } else { ?>
		    买家确认收货后，立即获得<?php  echo $this->set['texts']['commission1']?>
		    <?php  } ?>
		    <?php  if($this->set['withdraw']>0) { ?><br/>注意：可用<?php  echo $this->set['texts']['commission']?>满 <span class='text-orange'><?php  echo $this->set['withdraw']?><?php  echo $this->set['texts']['yuan']?></span> 后才能申请<?php  echo $this->set['texts']['withdraw']?><?php  } ?>
		    </div>
		</div>

	    </div>
	</div>
	  <!--<a <?php  if($cansettle ) { ?>href="<?php  echo mobileUrl('commission/apply')?>"<?php  } ?> class="btn btn-danger block<?php  if(!$cansettle ) { ?> disabled<?php  } ?>">我要提现</a>-->

	</div>
	<div class="fui-navbar">
		<a <?php  if($cansettle ) { ?>href="<?php  echo mobileUrl('commission/apply')?>"<?php  } ?> class="nav-item btn btn-warning block<?php  if(!$cansettle ) { ?> disabled<?php  } ?>">我要提现</a>
	</div>
    </div>
