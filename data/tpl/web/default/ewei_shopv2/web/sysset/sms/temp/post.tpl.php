...
<div class="page-heading">
	...
	<h2><?php  if(!empty($item['id'])) { ?><?php if(cv('sysset.sms.temp.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?><?php  } else { ?>添加<?php  } ?>短信模板 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['name'];?>】<?php  } ?></small></h2>
</div>
	...
	<div class="col-sm-9">
	 ...
	 <form <?php if( ce('sysset.sms.temp' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>" >模板名称</label>
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>">服务商</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('sysset.sms.temp' ,$item) ) { ?>
                            <?php  if(!empty($smsset['juhe']) || (!empty($item) && $item['type']=='juhe')) { ?>
                                <label class="radio-inline"><input type="radio" class="sms-type" data-template="1" name="type" value="juhe" <?php  if($item['type']=='juhe' || empty($item)) { ?>checked<?php  } ?> <?php  if(!empty($item['type'])) { ?>disabled<?php  } ?>> 聚合数据</label>
                            <?php  } ?>
                            ...
                            <?php  if(!empty($smsset['aliyun']) || (!empty($item) && $item['type']=='aliyun')) { ?>
                                <label class="radio-inline"><input type="radio" class="sms-type" data-template="1" name="type" value="aliyun" <?php  if($item['type']=='aliyun') { ?> checked<?php  } ?> <?php  if(!empty($item['type'])) { ?>disabled<?php  } ?>> 阿里云短信</label>
                            <?php  } ?>
                            <?php  if(!empty($smsset['emay']) || (!empty($item) && $item['type']=='emay')) { ?>
                                <label class="radio-inline"><input type="radio" class="sms-type" data-template="0" name="type" value="emay" <?php  if($item['type']=='emay') { ?> checked<?php  } ?> <?php  if(!empty($item['type'])) { ?>disabled<?php  } ?>> 亿美软通</label>
                            <?php  } ?>
                        ...
                        <?php  } else { ?>
                            <div class='form-control-static'><?php  if($item['type']=='juhe') { ?>聚合数据<?php  } else if($item['type']=='dayu') { ?>阿里大于<?php  } else if($item['type']=='emay') { ?>亿美软通<?php  } ?></div>
                        <?php  } ?>
                    </div>
                </div>

                <div class="form-group sms-template-1" style="<?php  if(!empty($item['template']) || empty($item)) { ?>display:block;<?php  } else { ?>display:none;<?php  } ?>">
                    <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>" >模板ID</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('sysset.sms.temp' ,$item) ) { ?>
                ...
                <div class="form-group sms-template-sign" style="<?php  if($item['type']=='dayu' || $item['type']=='aliyun' || $item['type']=='emay') { ?>display:block;<?php  } else { ?>display:none;<?php  } ?>">
                    <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>" >短信签名</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('sysset.sms.temp' ,$item) ) { ?>
                        <input type="text" name="smssign" class="form-control" value="<?php  echo $item['smssign'];?>" placeholder="" data-rule-required='true' />
                        <div class="help-block">请填写短信签名(如果服务商是大于\阿里云短信请填写审核成功的签名)</div>
                        <?php  } else { ?>
                ...
                <div class="form-group sms-template-0" style="<?php  if(empty($item['template']) && !empty($item)) { ?>display:block;<?php  } else { ?>display:none;<?php  } ?>">
                    <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>" >短信内容</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('sysset.sms.temp' ,$item) ) { ?>
        ...
        <div class="form-group">
            <label class="col-sm-2 control-label <?php if(cv('sysset.sms.temp.edit')) { ?>must<?php  } ?>">状态</label>
            <div class="col-sm-9 col-xs-12">
                <?php if( ce('sysset.sms.temp' ,$item) ) { ?>
...
<script language='javascript'>
    ...
    $(function () {
        $(".sms-type").unbind('click').click(function () {
            ...
            if(val=='dayu' || val=='aliyun' || val=='emay'){
    ...
    var kw = 1;
    function addType() {
        $(".btn-add-type").html("loading").addClass('disabled');
        $.ajax({
            url: "<?php  echo webUrl('sysset/sms/temp/tpl')?>&kw="+kw,
            cache: false
        }).done(function (html) {
            $(".btn-add-type").html('<i class="fa fa-plus" title=""></i> 增加一条数据值').removeClass('disabled');
            $("#datas").append(html);
        });
        kw++;
    }
