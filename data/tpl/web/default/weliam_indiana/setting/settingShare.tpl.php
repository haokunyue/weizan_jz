<?php defined('IN_IA') or exit('Access Denied');?><div class="tab-pane active" id="tab_share">
	<div class="form-group">
		<label class="col-xs-12 col-sm-3 col-md-2 col-lg-2 control-label">分享标题</label>
		<div class="col-sm-8">
			<input type="text" name="share_title" class="form-control" value="<?php  echo $settings['share_title'];?>" />
		</div>
	</div>
	
	<div class="form-group">
		<label class="col-xs-12 col-sm-3 col-md-2 col-lg-2 control-label">分享图片</label>
		<div class="col-sm-8">
			<?php  echo tpl_form_field_image('share_image', $settings['share_image']);?>
		</div>
	</div>
	<div class="form-group">
		<label class="col-xs-12 col-sm-3 col-md-2 col-lg-2 control-label">分享描述</label>
		<div class="col-sm-8">
			<input type="text" name="share_desc" class="form-control" value="<?php  echo $settings['share_desc'];?>" />
		</div>
	</div>
</div>