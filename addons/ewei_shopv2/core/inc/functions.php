...
if( !function_exists("p") ) 
{
	function p($name = "") 
	{
		...
		else 
		{
			...
			{
				if( $name == "seckill" && (!function_exists("redis") || is_error(redis())) ) 
				{
					return false;
				}
				$return_instance = true;
...
if( !function_exists("com_perm_check_plugin") ) 
{
	function com_perm_check_plugin($pluginname = "") 
	{
		...
		if( ($_W["role"] == "founder" || empty($_W["role"])) && in_array($owner["uid"], $founders) ) 
		{
			return true;
		}
...
if( !function_exists("com_perm_check_com") ) 
{
	function com_perm_check_com($comname) 
	{
		...
		$founders = explode(",", $_W["config"]["setting"]["founder"]);
		$owner = account_owner($_W["uniacid"]);
		if( $_W["role"] == "founder" && in_array($owner["uid"], $founders) ) 
		{
			return true;
		}
		if( empty($_W["role"]) ) 
...
if( !function_exists("shop_template_parse") ) 
{
	function shop_template_parse($str, $inmodule = false) 
	{
		...
		if( (strexists($_W["siteurl"], "merchant.php") || strexists($_W["siteurl"], "r=merch.mmanage")) && p("merch") ) 
		{
		...
		if( strexists($_W["siteurl"], "newstoreant.php") && p("newstore") ) 
		{
			$str = preg_replace("/{ifp\\s+(.+?)}/", "<?php if(mcv(\$1)) { ?>", $str);
			$str = preg_replace("/{ifpp\\s+(.+?)}/", "<?php if(mcp(\$1)) { ?>", $str);
			$str = preg_replace("/{ife\\s+(\\S+)\\s+(\\S+)}/", "<?php if( mce(\$1 ,\$2) ) { ?>", $str);
...
if( !function_exists("cv") ) 
{
	function cv($permtypes = "") 
	{
		if( !com("perm") ) 
		{
			...
			if( $arr[0] == "sale" && !com_perm_check_com("sale") && !com_perm_check_com("coupon") && !com_perm_check_com("wxcard") ) 
			{
				return false;
			}
			return true;
...
if( !function_exists("tpl_selector") ) 
{
	function tpl_selector($name, $options = array( )) 
	{
		...
		else 
		{
			...
			{
				if( $options["type"] == "coupon_cp" ) 
				{
					$html .= "<div class='input-group multi-audio-details' " . $show . ">\r\n                        <table class='table'>\r\n                            <thead>\r\n                            <tr>\r\n                                <th style='width:100px;'>优惠券名称</th>\r\n                                <th style='width:200px;'></th>\r\n                                <th></th>\r\n                                <th></th>\r\n                                <th style='width:80px;'>操作</th>\r\n                            </tr>\r\n                            </thead>\r\n                            <tbody id='param-items' class='ui-sortable container'>";
				}
				else 
				{
					if( $options["type"] == "coupon_share" ) 
					{
						$html .= "<div class='input-group multi-audio-details' " . $show . ">\r\n                        <table class='table'>\r\n                            <thead>\r\n                            <tr>\r\n                                <th style='width:100px;'>优惠券名称</th>\r\n                                <th style='width:200px;'></th>\r\n                                <th></th>\r\n                                <th>每人领取数量</th>\r\n                                <th style='width:80px;'>操作</th>\r\n                            </tr>\r\n                            </thead>\r\n                            <tbody id='param-items' class='ui-sortable container'>";
					}
					else 
					{
						if( $options["type"] == "coupon_shares" ) 
						{
							$html .= "<div class='input-group multi-audio-details' " . $show . ">\r\n                        <table class='table'>\r\n                            <thead>\r\n                            <tr>\r\n                                <th style='width:100px;'>优惠券名称</th>\r\n                                <th style='width:200px;'></th>\r\n                                <th></th>\r\n                                <th>每人领取数量</th>\r\n                                <th style='width:80px;'>操作</th>\r\n                            </tr>\r\n                            </thead>\r\n                            <tbody id='param-items' class='ui-sortable container'>";
						}
						else 
				...
				if( $options["type"] == "product" ) 
				{
					...
					if( $item["optiontitle"] ) 
					{
						...
						foreach( $item["optiontitle"] as $option ) 
						{
							...
							$html .= "<input type='hidden' value='" . $option["packageprice"] . "," . $option["commission1"] . "," . $option["commission2"] . "," . $option["commission3"] . (string) $total . $maxbuy . $totalmaxbuy . "'\r\n                        name='" . $options["selectorid"] . "packagegoodsoption" . $option["optionid"] . "' >";
						}
					}
					else 
					{
...
if( !function_exists("webUrl") ) 
{
	function webUrl($do = "", $query = array( ), $full = true) 
	{
		...
		if( !empty($_W["plugin"]) ) 
		{
			if( $_W["plugin"] == "merch" && function_exists("merchUrl") ) 
			{
				return merchUrl($do, $query, $full);
			}
			if( $_W["plugin"] == "newstore" && function_exists("newstoreUrl") ) 
			{
...
if( !function_exists("save_media") ) 
{
	function save_media($url, $enforceQiniu = false) 
	{
		...
		if( !empty($_W["setting"]["remote"]["type"]) && !empty($url) && !(strexists($url, "http:") || strexists($url, "https:")) && is_file(ATTACHMENT_ROOT . $url) ) 
		{
			load()->func("file");
