...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function submit() 
	{
		...
		foreach( $goods as $g ) 
		{
			...
			if( !empty($_SESSION["exchange"]) && p("exchange") && (empty($data["status"]) || !empty($data["deleted"])) ) 
			{
		...
		if( !empty($addressid) && $dispatchtype == 0 && !$isonlyverifygoods ) 
		{
			...
			else 
			{
				...
				$area_set = m("util")->get_area_config_set();
				$new_area = intval($area_set["new_area"]);
				if( !empty($new_area) && (empty($address["datavalue"]) || trim($address["datavalue"] == "null null null")) ) 
				{
