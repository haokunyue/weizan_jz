define(['jquery.ui'], function (ui) {
    ...
    modal.initNavs = function () {
        modal.setNavs();
        var navgroup = {
            0: ['listmenu', 'richtext', 'title', 'line', 'blank', 'menu', 'menu2', 'picture', 'banner', 'picturew', 'pictures', 'icongroup', 'audio', 'coupon', 'video'],
            2: ['notice', 'goods', 'search', 'fixedsearch']
        };
    ...
    modal.initClick = function () {
        $(".btn-save").unbind('click').click(function () {
            ...
            if (type == 'isdefault') {
                tip.confirm('确定要保存并设为默认首页吗?', function () {
                    modal.save(true)
                })
            } else if (type == 'save') {
                modal.save()
    ...
    modal.initEditor = function (scroll, activetab) {
        ...
        if ($("#diy-editor .goods-selector").length > 0) {
            ...
            var url = biz.url('app/page/goods_query', null, modal.merch);
    ...
    modal.save = function (isdefault) {
        ...
        $.post(biz.url(modal.id > 0 ? 'app/page/edit' : 'app/page/add'), {
            ...
            $(".btn-save[data-type='isdefault']").text("保存并设为默认首页").removeAttr('stop');
            if (ret.status != 1) {
                tip.msgbox.err(ret.result.message);
                return
            }
            tip.msgbox.suc(isdefault ? '保存成功并设为默认首页！' : '保存成功！');
