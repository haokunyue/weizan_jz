...
class Release_EweiShopV2Page extends PluginWebPage
{
	...
	public function main()
	{
		...
		if (is_error($auth)) {
			...
			$authUrl = EWEI_SHOPV2_AUTH_WXAPP . 'auth/auth?uniacid=' . $auth['id'];
	...
	public function audit()
	{
		...
		if ($action == 'upload') {
			...
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . 'index/platform/audit?uniacid=' . $_W['uniacid'], array('tabBar' => $tabBar));
		}
		else {
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . 'index/platform/submit?uniacid=' . $_W['uniacid'], array());
		}
		if ($request['code'] != 200) {
	...
	public function auth()
	{
		...
		header('Location:' . EWEI_SHOPV2_AUTH_WXAPP . 'index/platform/auth?uniacid=' . $_W["uniacid"]);
	}
