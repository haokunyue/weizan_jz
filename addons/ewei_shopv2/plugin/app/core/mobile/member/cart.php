...
class Cart_EweiShopV2Page extends AppMobilePage 
{
	...
	public function add() 
	{
		...
		$goods = pdo_fetch('select id,marketprice,total,diyformid,diyformtype,diyfields, isverify, type,merchid from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
