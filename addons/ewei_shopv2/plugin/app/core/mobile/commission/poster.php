...
class Poster_EweiShopV2Page extends Base_EweiShopV2Page 
{
	...
	public function getimage() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			switch ($item['type']) 
			{
				case 'qrcode': $qrcode = $this->getQR($poster, $member);
				if (is_error($qrcode)) 
				{
					break;
				}
				...
				break;
				case 'avatar': $avatar = preg_replace('/\\/0$/i', '/96', $member['avatar']);
				$target = $this->mergeImage($target, $item, $avatar);
				break;
				case 'nickname': $target = $this->mergeText($target, $item, $member['nickname']);
				break;
			}
		}
	...
	private function mergeImage($target = false, $data = array(), $imgurl = '', $local = false) 
	{
		...
		if ($data['type'] == 'avatar') 
		{
			switch ($data['size']) 
			{
				case 'big': $sizes = array('width' => 150, 'height' => 150);
				break;
				case 'medium': $sizes = array('width' => 120, 'height' => 120);
				break;
				case 'small': $sizes = array('width' => 90, 'height' => 90);
				break;
			}
		}
		else if ($data['type'] == 'qrcode') 
		{
			switch ($data['size']) 
			{
				case 'big': $sizes = array('width' => 240, 'height' => 240);
				break;
				case 'medium': $sizes = array('width' => 200, 'height' => 200);
				break;
				case 'small': $sizes = array('width' => 160, 'height' => 160);
				break;
			}
		}
	...
	private function mergeText($target = false, $data = array(), $text = '') 
	{
		...
		switch ($data['size']) 
		{
			case 'big': $fontsize = 32;
			break;
			case 'medium': $fontsize = 28;
			break;
