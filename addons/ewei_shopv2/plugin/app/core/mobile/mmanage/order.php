...
class Order_EweiShopV2Page extends AppMobileAuthPage 
{
	...
	{
		app_json(array( 'perm' => array('order0' => cv('order.list.status0'), 'order1' => cv('order.list.status1'), 'order2' => cv('order.list.status2'), 'order3' => cv('order.list.status3'), 'order_1' => cv('order.list.status_1'), 'order4' => cv('order.list.status4'), 'order5' => cv('order.list.status5'), 'refund' => cv('order.op.refund')) ));
	}
	public function get_list() 
	{
		...
		$total = $t['count'];
		app_json(array( 'list' => $list, 'total' => $total, 'pagesize' => $psize, 'perm' => array('order_pay' => cv('order.op.pay'), 'order_send' => cv('order.op.send'), 'order_verify' => cv('order.op.verify'), 'order_fetch' => cv('order.op.fetch'), 'order_finish' => cv('order.op.finish'), 'order_sendcancel' => cv('order.op.sendcancel'), 'order_remarksaler' => cv('order.op.remarksaler')) ));
	}
	public function get_detail() 
	{
		...
		unset($item['transid'], $item['carrier'], $item['iscomment'], $item['creditadd']);
		...
		app_json(array( 'detail' => $item, 'member' => array('id' => $member['id'], 'nickname' => $member['nickname'], 'realname' => $member['realname'], 'avatar' => tomedia($member['avatar']), 'mobile' => $member['mobile'], 'weixin' => $member['weixin']), 'dispatch' => $dispatch, 'user_address' => $user_address, 'agents' => $agents, 'coupon' => $coupon, 'order_fields' => $order_fields, 'order_data' => $order_data, 'order_goods' => $order_goods, 'verifyinfo' => $verifyinfo, 'store' => $store, 'saler' => $saler ));
	}
	...
	public function changeprice() 
	{
		...
		app_json(array('list' => $order_goods, 'item' => $item));
	}
}
?>