...
class Index_EweiShopV2Page extends AppMobileAuthPage 
{
	...
	public function home() 
	{
		...
		$result = array( 'shopset' => array('name' => $_W['shopset']['shop']['name'], 'logo' => tomedia($_W['shopset']['shop']['logo']), 'desc' => $_W['shopset']['shop']['description']), 'notices' => $notices, 'today_count' => $order['count'], 'today_price' => $order['price'], 'today_member' => $member['count'], 'order_count' => m('order')->getTotals(), 'goods_count' => $goodscount, 'member_count' => $member_count, 'perms' => array('sysset' => cv('sysset'), 'order_status0' => cv('order.list.status0'), 'order_status1' => cv('order.list.status1'), 'order_status2' => cv('order.list.status2'), 'order_status4' => cv('order.list.status4'), 'order_status5' => cv('order.list.status5'), 'goods' => cv('goods'), 'member' => cv('member'), 'finance' => cv('finance'), 'statistics' => cv('statistics')) );
		app_json($result);
	}
