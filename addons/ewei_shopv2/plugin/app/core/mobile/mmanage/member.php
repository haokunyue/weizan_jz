...
class Member_EweiShopV2Page extends AppMobileAuthPage 
{
	...
	{
		app_json(array( 'perm' => array('member' => cv('member')) ));
	}
	public function get_list() 
	{
		...
		app_json(array( 'total' => $total, 'list' => $listArr, 'pagesize' => $psize, 'page' => $pindex, 'perm' => array('member_view' => cv('member.list.view'), 'member_edit' => cv('member.list.edit')) ));
	}
	public function get_detail() 
	{
		...
		if ($plugin_commission) 
		{
			...
			if (!(empty($member['agentlevel'])) && !(empty($agentlevels))) 
			{
				...
				{
					while ($member['agentlevel'] == $agentlevel['id']) 
					{
		...
		$groups = array_merge(array( array('id' => 0, 'groupname' => '未分组') ), $groups);
		$levels = m('member')->getLevels();
		$levels = array_merge(array( array('id' => 0, 'levelname' => (empty($_W['shopset']['shop']['levelname']) ? '普通会员' : $_W['shopset']['shop']['levelname'])) ), $levels);
		$shop = $_W['shopset']['shop'];
