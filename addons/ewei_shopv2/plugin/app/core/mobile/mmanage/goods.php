...
class Goods_EweiShopV2Page extends AppMobileAuthPage 
{
	...
	{
		app_json(array( 'perm' => array('goods_add' => cv('goods.add'), 'goods_view' => cv('goods.view'), 'goods_edit' => cv('goods.edit'), 'goods_status' => cv('goods.status'), 'goods_delete' => cv('goods.delete'), 'goods_restore' => cv('goods.restore'), 'goods_stock' => cv('goods.stock')) ));
	}
	...
	public function get_detail() 
	{
		...
		$levels = array_merge(array( array('id' => 0, 'levelname' => (empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])) ), $levels);
		...
		app_json(array( 'goods' => $item, 'level_list' => $levels, 'group_list' => $groups, 'dispatch_list' => $dispatch_data, 'diyform_list' => $diyform_list, 'category_list' => $category, 'allcategory' => $allcategory, 'hasdiyform' => p('diyform'), 'hasvirtual' => com('virtual'), 'virtual_list' => $virtual_list, 'speclist' => $specs, 'optionarray' => $optionlist, 'specid' => $specid, 'spectitle' => $spectitle, 'perm' => array('goods_edit' => cv('goods.edit')) ));
	}
	public function submit() 
	{
		...
		else if ($_GPC['verifygoodslimittype'] == '1') 
		{
