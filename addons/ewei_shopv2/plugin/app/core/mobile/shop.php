...
class Shop_EweiShopV2Page extends AppMobilePage 
{
	public function get_shopindex() 
	{
		...
		global $_GPC;
		$uniacid = $_W['uniacid'];
		$defaults = array( 'adv' => array('text' => '幻灯片', 'visible' => 1), 'search' => array('text' => '搜索栏', 'visible' => 1), 'nav' => array('text' => '导航栏', 'visible' => 1), 'notice' => array('text' => '公告栏', 'visible' => 1), 'cube' => array('text' => '魔方栏', 'visible' => 1), 'banner' => array('text' => '广告栏', 'visible' => 1), 'recommand' => array('text' => '推荐栏', 'visible' => 1) );
		$appsql = '';
		if ($this->iswxapp) 
		{
	...
	public function get_category() 
	{
		...
		foreach ($category['children'] as $k => $v ) 
		{
			foreach ($v as $r ) 
			{
				if ($r['isrecommand'] == 1) 
				{
					...
					$rec = array( 'id' => $r['id'], 'name' => $r['name'], 'thumb' => $r['thumb'], 'advurl' => $r['advurl'], 'advimg' => $r['advimg'], 'child' => array(), 'level' => $r['level'] );
					if (isset($category['children'][$r['id']])) 
					{
						foreach ($category['children'][$r['id']] as $c ) 
						{
							$c['thumb'] = tomedia($c['thumb']);
							$child = array( 'id' => $c['id'], 'name' => $c['name'], 'thumb' => $c['thumb'], 'advurl' => $c['advurl'], 'advimg' => $c['advimg'], 'child' => array() );
							$rec['child'][] = $child;
						}
					}
					$recommands[] = $rec;
		...
		foreach ($category['parent'] as $p ) 
		{
			...
			$parent = array( 'id' => $p['id'], 'name' => $p['name'], 'thumb' => $p['thumb'], 'advurl' => $p['advurl'], 'advimg' => $p['advimg'], 'child' => array() );
			if (is_array($category['children'][$p['id']])) 
			{
				foreach ($category['children'][$p['id']] as $c ) 
				{
					...
					if (!(empty($c['id']))) 
					{
						$child = array( 'id' => $c['id'], 'name' => $c['name'], 'thumb' => $c['thumb'], 'advurl' => $c['advurl'], 'advimg' => $c['advimg'], 'child' => array(), 'level' => $c['level'] );
					}
	 ...
	 public function get_danmus()
	{
		global $_W;
		
			$danmu['data'] = pdo_fetchall('SELECT m.nickname, m.avatar as imgurl, o.createtime as time FROM' . tablename('ewei_shop_member') . ' o LEFT JOIN ' . tablename('ewei_shop_member') . ' m ON m.openid=o.openid WHERE o.uniacid=:uniacid AND m.nickname!=\'\' AND m.avatar!=\'\' ORDER BY o.createtime ASC LIMIT 30', array(':uniacid' => $_W['uniacid']));
		$danmu['data'][$index]['time'] = p('diypage')->getDanmuTime($time);
    $danmutime=$danmu['data'][$index]['time'];
	
     $hidedddm = $_W['shopset']['app']['hidedddm'];
   app_json(array('danmu' => $danmu['data'] ,'danmutime' => $danmutime,'hidedddm' => $hidedddm));
  }
	...
	public function get_set() 
	{
		...
		$sets = array( 'shop' => array('name' => $global_set['shop']['name'], 'logo' => tomedia($global_set['shop']['logo']), 'description' => $global_set['shop']['description'], 'img' => tomedia($global_set['shop']['img'])), 'share' => array('title' => (empty($global_set['share']['title']) ? $global_set['shop']['name'] : $global_set['share']['title']), 'img' => (empty($global_set['share']['icon']) ? tomedia($global_set['shop']['logo']) : tomedia($global_set['share']['icon'])), 'desc' => (empty($global_set['share']['desc']) ? $global_set['shop']['description'] : $global_set['share']['desc']), 'link' => (empty($global_set['share']['url']) ? mobileUrl('', array('appfrom' => 1), true) : $global_set['share']['url'])), 'trade' => array('closerecharge' => intval($global_set['trade']['closerecharge']), 'minimumcharge' => floatval($global_set['trade']['minimumcharge']), 'withdraw' => intval($global_set['trade']['withdraw']), 'withdrawmoney' => floatval($global_set['trade']['withdrawmoney']), 'closecomment' => intval($global_set['trade']['withdraw']), 'closecommentshow' => intval($global_set['trade']['closecommentshow'])), 'payset' => array('weixin' => intval($global_set['pay']['weixin']), 'alipay' => intval($global_set['pay']['alipay']), 'credit' => intval($global_set['pay']['credit']), 'cash' => intval($global_set['pay']['cash'])), 'contact' => array('phone' => (isset($global_set['contact']['phone']) ? $global_set['contact']['phone'] : ''), 'province' => (isset($global_set['contact']['phone']) ? $global_set['contact']['province'] : ''), 'city' => (isset($global_set['contact']['phone']) ? $global_set['contact']['city'] : ''), 'address' => (isset($global_set['contact']['phone']) ? $global_set['contact']['address'] : '')), 'menu' => $this->model->diyMenu('shop'), 'cancelorderreasons' => array('不取消了', '我不想买了', '信息填写错误，重新拍', '同城见面交易', '其他原因'), 'openmerch' => $openmerch, 'texts' => array('credittext' => $global_set['trade']['credittext'], 'moneytext' => $global_set['trade']['moneytext']) );
