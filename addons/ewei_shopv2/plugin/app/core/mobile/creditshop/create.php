...
class Create_EweiShopV2Page extends AppMobilePage 
{
	public function main() 
	{
		...
		if (empty($goods)) 
		{
			app_error(AppError::$GoodsSoldOut, '商品已下架！');
		}
		...
		app_json(array('goods' => $goods, 'is_openmerch' => $is_openmerch, 'stores' => $stores, 'address' => $address, 'shop' => $shop, 'member' => $member));
	}
	...
	public function dispatch() 
	{
		...
		if (!(empty($nodispatch_array))) 
		{
			...
			app_json(array('nodispatch_array' => $nodispatch_array));
		}
		app_json(array('dispatch' => $dprice));
	}
}
?>