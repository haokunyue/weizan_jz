...
class Comment_EweiShopV2Page extends AppMobilePage 
{
	public function __construct() 
	{
		...
		if (!(empty($trade['closecomment']))) 
		{
			...
			app_error(AppError::$OrderCanNotComment, '不允许评论!');
		}
	}
	public function main() 
	{
		...
		if (empty($log)) 
		{
			app_error(AppError::$ParamsError, '订单不存在或已删除!');
		}
		...
		if ($log['goodstype'] == 0) 
		{
			if ($log['status'] != 3) 
			{
				app_error(AppError::$OrderNndone, '订单未完成，不能评价!');
			}
		}
		...
		if (2 <= $log['iscomment']) 
		{
			...
			app_error(AppError::$OrderNndone, '您已经评价过了!');
		}
		app_json(array('is_openmerch' => $is_openmerch, 'log' => $log, 'shop' => $_W['shopset']['shop']));
	}
	public function submit() 
	{
		...
		if (empty($log)) 
		{
			app_error(AppError::$RecordNotFound, '兑换记录未找到');
		}
		$member = m('member')->getMember($openid);
		$comments = $_GPC['comments'];
		if (!(is_array($comments))) 
		{
			app_error(AppError::$RecordNotFound, '数据出错，请重试!');
		}
		$trade = m('common')->getSysset('trade');
