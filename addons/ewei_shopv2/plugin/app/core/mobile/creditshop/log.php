...
class Log_EweiShopV2Page extends AppMobilePage 
{
	public function getlist() 
	{
		...
		app_json(array('list' => $list, 'pagesize' => $psize, 'total' => $total, 'next_page' => ceil($total / $psize)));
	}
	public function detail() 
	{
		...
		if (empty($log)) 
		{
			app_error(AppError::$ExchangeRecordNotFound, '兑换记录不存在！');
		}
		...
		if (empty($goods['id'])) 
		{
			app_error(AppError::$GoodsNotFound, '商品记录不存在！');
		}
		$address = false;
		...
		app_json(array('store' => $store, 'set' => $set, 'stores' => $stores, 'goods' => $goods, 'verifynum' => $verifynum, 'ordercredit' => $ordercredit, 'ordermoney' => $ordermoney, 'address' => $address, 'carrier' => $carrier, 'shop' => $_W['shopset']['shop'], 'verify' => $verify, 'log' => $log));
	}
	public function Receivepacket() 
	{
		...
		if (!($log)) 
		{
			app_error(AppError::$logNotFound, '该订单不存在或已删除！');
		}
		if ((2 < $log['status']) && (0 < $log['time_finish'])) 
		{
			app_error(AppError::$PacketGet, '红包已领取！');
		}
		if ($log['status'] < 2) 
		{
			app_error(AppError::$PacketDissatisfyCondition, '红包为满足领取条件！');
		}
		$packet = p('creditshop')->packetmoney($log['goodsid']);
		if (!($packet['status'])) 
		{
			app_json(array('message' => $packet['message']));
		}
		...
		{
			app_error(AppError::$MoneyInsufficient, '剩余金额不足，请联系管理员！');
		}
		$err = m('common')->sendredpack($params);
		if (is_error($err)) 
		{
			app_error(AppError::$PacketError, '红包发放出错，请联系管理员！');
		}
		else 
		{
	...
	public function finish() 
	{
		...
		if (empty($log)) 
		{
			app_error(AppError::$ExchangeRecordNotFound, '订单未找到');
		}
		if (($log['status'] != 3) && empty($log['expresssn'])) 
		{
			app_error(AppError::$OrderNotTake, '订单不能确认收货');
		}
	...
	public function paydispatch() 
	{
		...
		app_json(array('logid' => $log['id'], 'wechat' => $wechat, 'jssdkconfig' => json_encode($_W['account']['jssdkconfig'])));
	}
