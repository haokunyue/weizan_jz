	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			else 
			{
				...
				if (!(empty($optionid))) 
				{
					$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,`virtual`,stock,weight,specs,isfullback,`day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
					if (!(empty($option))) 
					{
						$data['optionid'] = $optionid;
		...
		foreach ($goods as $g ) 
		{
			...
			if ($data['type'] == 5) 
			{
				if ($data['verifygoodslimittype'] == 1) 
				{
					...
					else if ($data['verifygoodslimittype'] == 0) 
					{
