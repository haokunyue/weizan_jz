...
if ($do == 'module_detail') {
	...
	$modules = pdo_get('modules', array('name' => $module_name));
	if(checksubmit('submit')) {
			$update = array(
				'template_id' => $_GPC['template_id'],
			);
			pdo_update('modules', $update, array('name' => $module_name));
            itoast('设置成功！', url('module/manage-system', array('account_type' => '4')), 'success');
    }
}
