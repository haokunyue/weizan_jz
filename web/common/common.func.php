...
function frames_menu_append() {
	$system_menu_default_permission = array(
		...
		'owner' => array(
			...
			'system_shop_member_member',
			'system_shop_member_record',
			'system_shop_member_chongzhi',
			'user_set_yuming',
			'user_set_copyright',
			'user_set_pifu',
		),
...
//                  '<a href="{$url}" target="_blank">请尽快完善您在系统云服务平台的站点注册信息。</a>'+
//            '</div>'+
//      '</div>'+
//        '</div>';
//	$('body').prepend(html);
...
function site_safety_monitoring(){
	...
	if ($_W['isfounder']) {
			...
			if($safe=='safe'){
			$url = 'http://www.efwww.com/forum.php?mod=viewthread&tid=2512';
			return <<<EOF
$(function() {
//	var html = 
